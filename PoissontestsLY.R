...
Poisson_runs_LY <- function(startinglifeyears,diseasereduction,relative_effect) 
{
  ...
  LP_Poisson_LY <-exp(intercept+(b1*x[,1])+(b2*x[,2])+(b3*x[,3])+(b4*x[,4])+(b5*x[,5]))*LY
  
  
  
  #2nd stage
  Poissonmodel_stage2 <- glm(y ~ w,offset = LP_Poisson_LY, family = poisson(link = "log")) 
  ...
  Poisson_w = exp(Poissonmodel_stage2$coefficients[[1]]+Poissonmodel_stage2$coefficients[[2]]*w + (LP_Poisson_LY))
  Poisson_wflipped = exp(Poissonmodel_stage2$coefficients[[1]]+Poissonmodel_stage2$coefficients[[2]]*w.rev +  (LP_Poisson_LY))
